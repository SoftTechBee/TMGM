...
public partial class Admin_SupportLIst : System.Web.UI.Page
{
    ...
    public void loadlist()
    {
        try
        {
            string sql = "select * from tblsupport where  status='Approved'  order by DOI desc";
    ...
    protected void btnSeach_Click(object sender, EventArgs e)
    {
        try
        {
            string sql = "select * from tblsupport where username='" + txtUname.Text + "' ";
            DataTable dt = objcon.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
            ...
            {
                lbdanger.Text = "Opps! NO USER Found";
                danger.Visible = true;
            }
