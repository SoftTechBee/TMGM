...
public partial class Admin_withdrawApprovedlist : System.Web.UI.Page
{
    ...
    {
        try {
            string sql = "select r.Wallet,r.Packtype,r.Rid, r.username,r.Amount,r.DOR,r.DOA,r.remark,r.status,b.*,r.Payout,r.AdminCharge from TblRWithdraw  r Left Join register b on r.Username=b.Username where r.status='Approved'";
            if (username != "")
            {
